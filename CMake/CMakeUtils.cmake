...
function(utils_include_directories)
  set(__add_before "")
  foreach(dir ${ARGN})
...
macro(utils_clear_vars)
  foreach(_var ${ARGN})
    unset(${_var} CACHE)
  endforeach()
endmacro()

set(UTILS_COMPILER_FAIL_REGEX
...
MACRO(utils_check_compiler_flag LANG FLAG RESULT)
  if(NOT DEFINED ${RESULT})
    ...
    if(_fname)
      ...
      FOREACH(_regex ${UTILS_COMPILER_FAIL_REGEX})
...
macro(utils_check_flag_support lang flag varname)
  if("_${lang}_" MATCHES "_CXX_")
    set(_lang CXX)
  elseif("_${lang}_" MATCHES "_C_")
    set(_lang C)
  ...
  utils_check_compiler_flag("${_lang}" "${ARGN} ${flag}" ${${varname}})
endmacro()

# turns off warnings
macro(utils_warnings_disable)
    ...
    elseif((CMAKE_COMPILER_IS_GNUCXX OR (UNIX AND CV_ICC)) AND _gxx_warnings AND _flag_vars)
      foreach(var ${_flag_vars})
        foreach(warning ${_gxx_warnings})
          ...
          utils_check_flag_support(${var} "${warning}" _varname)
          if(${_varname})
            set(${var} "${${var}} ${warning}")
          endif()
        endforeach()
...
macro(UTILS_OPTION variable description value)
  set(__value ${value})
  set(__condition "")
...
function(utils_output_status msg)
  message(STATUS "${msg}")
  ...
  if(DEFINED status_cond)
    ...
    if(DEFINED status_then OR DEFINED status_else)
      if(${status_cond})
        ...
        utils_output_status("${status_text} ${status_then}")
      else()
        string(REPLACE ";" " " status_else "${status_else}")
        string(REGEX REPLACE "^[ \t]+" "" status_else "${status_else}")
        utils_output_status("${status_text} ${status_else}")
      endif()
    else()
      string(REPLACE ";" " " status_cond "${status_cond}")
      string(REGEX REPLACE "^[ \t]+" "" status_cond "${status_cond}")
      utils_output_status("${status_text} ${status_cond}")
    endif()
  else()
    utils_output_status("${text}")
  endif()
...
macro(utils_split_libs_list lst lstdbg lstopt)
  set(${lstdbg} "")
  set(${lstopt} "")
  set(perv_keyword "")
  foreach(word ${${lst}})
...
macro(utils_list_filterout lst regex)
  foreach(item ${${lst}})
...
macro(utils_list_reverse __lst)
  if(${__lst})
    list(REVERSE ${__lst})
  endif()
endmacro()
...
macro(utils_list_add_prefix LST PREFIX)
  set(__tmp "")
  foreach(item ${${LST}})
...
macro(utils_list_add_suffix LST SUFFIX)
  set(__tmp "")
  foreach(item ${${LST}})
...
macro(utils_list_pop_front LST VAR)
  if(${LST})
    list(GET ${LST} 0 ${VAR})
...
macro(utils_convert_to_full_paths VAR)
  if(${VAR})
    set(__tmp "")
    foreach(path ${${VAR}})
...
macro(utils_parse_header FILENAME FILE_VAR)
  set(vars_regex "")
...
macro(utils_parse_header2 LIBNAME HDR_PATH VARNAME)
  utils_clear_vars(${LIBNAME}_VERSION_MAJOR
...
function(utils_source_group group)
  cmake_parse_arguments(UTILS_SOURCE_GROUP "" "" "GLOB" ${ARGN})
  file(GLOB srcs ${UTILS_SOURCE_GROUP_GLOB})
